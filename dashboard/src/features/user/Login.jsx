...
function Login() {
  ...
  const submitForm = async () => {
    ...
    if (loginWithOtp) {
      ...
      // proceeding with login with password
      const validation = validator(formObj, {
        email: setErrorEmailMessage,
        password: setErrorPasswordMessage,
      }); // checks if any field is empty
      const validatePassword = validatePasswordMessage(
        formObj.password,
        setErrorPasswordMessage
      ); // check the password validations

      if (!validation || validEmail || !validatePassword) {
        //if any field is empty or if email format is not valid or password validation failed
        setLoading(false);
        return;
      }
          ...
          {otpSent ? (
            ...
            <form onSubmit={proceedLogin}>
              ...
              >
                ...
                />
                <Button
                  functioning={LoginWithOTP}
                  text={
                    loginWithOtp ? "Sign In with Password" : "Sign In With OTP"
                  }
                  classes={
                    "rounded-md text-[.8rem] mt-2 w-[22rem] h-[2.3rem] btn-stone hover:text-stone-50 hover:bg-stone-700 border border-stone-700 bg-stone-50 text-stone-800"
                  }
                />
              </div>
            </form>
