...
const RequestDetails = ({close}) => {
  ...
  const requestStageStyle =
    statusStyles[getStyle[version?.versionStatus]] || {};

  const handleRestoreVersionRequest = async (version) => {
    // validation
    if (version.isLive)
      return toast.error("Error! Version is already live.", toastObject);
    if (version.versionStatus !== "PUBLISHED")
      return toast.error(
        "Can't restore version. Version was never published.",
        toastObject
      );
    ...
    try {
      ...
      if (response.ok) {
        ...
        runToast("SUCCESS", "Version has been restored.");
        close();
        setRandom();
      } else {
        // toast.error("Failed to restore.", toastObject)
        runToast("ERROR", "Failed to restore.");
      }
    } catch (err) {
      console.log(err);
    }
    ...
    <div>
      <div className="flex flex-col h-[87%] text-[14px] custom-text-color p-[10px] py-0  mt-2 overflow-y-auto customscroller">
        <div className="flex py-[15px] justify-between border-b dark:border-[#8a8a8a]">
          ...
          <p
            className={` inline-flex items-center px-3 py-1 rounded-full text-xs font-small`}
          >
        ...
        {version.versionStatus === "PUBLISHED" && (
          <div className="flex py-[15px] justify-between border-b dark:border-[#8a8a8a]">
            <label></label>
            <button
              className="underline text-[#145098] dark:text-[#0EA5E9] font-[300]"
        ...
        <div className="flex flex-col">
          <div
            className={`flex py-[15px] justify-between items-center border-b dark:border-[#8a8a8a]`}
          >
            <label>Version Status</label>
            <div
              className={`${requestStageStyle.bg} ${requestStageStyle.text} w-max flex items-center px-1 py-[2px] rounded-2xl gap-[2.5px]`}
            >
        ...
        {version.scheduledAt && (
          ...
          >
            ...
            <div
              className={`${requestStageStyle.bg} ${requestStageStyle.text} w-max flex items-center px-2 py-[2px] rounded-2xl gap-[2.5px]`}
            >
              {/* {requestStageStyle.icon} */}
          ...
          <div className="">
                ...
                return (
                  <div
                    ...
                    className={`flex items-center justify-between py-1 border-b ${
                      lastIndex
                        ? "border-stone-[#e5e7eb] dark:border-[#8a8a8a]"
                        : "border-stone-[#efefef] dark:border-[#3d3d3d]"
                    }`}
                  >
                    <p className="w-[200px]">
                      {TruncateText(manager.user.name, 18)}
                    </p>
                    <p className="text-xs">
                      {formatTimestamp(manager.assignedAt, " ")}
                    </p>
                    <p
                      className={`w-[69px] text-center rounded-full px-1 text-white ${
            ...
            <div className=" dark:border-stone-700 flex-col justify-between py-2 items-center">
              ...
              {editors?.map((editor, i, a) => {
                ...
                return (
                  ...
                  >
                    <p className="w-[200px]">
                      {TruncateText(editor.user.name, 18)}
                    </p>
                    <p className="text-xs">
                      {formatTimestamp(editor.assignedAt, " ")}
                    </p>
                    <p
                      className={`w-[69px] text-center rounded-full px-1 text-white ${
            ...
            <div className="flex flex-col">
              ...
              {verifiers?.map((stage, ind, a) => {
                ...
                return (
                  <div
                    className={` flex-col justify-between py-2 items-center
                  ...
                  >
                    ...
                    {stage[1]?.map((verifier, i, a) => {
                      ...
                      return (
                        <div
                          key={verifier.user.id}
                          className={`flex items-center justify-between py-1 border-b 
                                                        ${
                                                          lastIndex &&
                                                          lastIndexVerifier
                                                            ? "border-b border-stone-[#e5e7eb] dark:border-[#8a8a8a]"
                                                            : "border-stone-[#efefef] dark:border-[#3d3d3d] "
                                                        }`}
                        >
                          <p className="w-[200px]">
                            {TruncateText(verifier.user.name, 18)}
                          </p>
                          <p className="text-xs">
                            {formatTimestamp(verifier.assignedAt, " ")}
                          </p>
                          <p
                            className={`w-[69px] text-center rounded-full px-1 text-white 
                                                            ${
                                                            ...
                                                            `}
                          >
                            {capitalizeWords(verifier.status)}
                          </p>
                        </div>
                      );
                    })}
            ...
            <div className=" dark:border-stone-700 flex-col justify-between py-2 items-center">
              ...
              {publishers?.map((publisher, i, a) => {
                ...
                return (
                  ...
                  >
                    <p className="w-[200px]">
                      {TruncateText(publisher.user.name, 18)}
                    </p>
                    <p className="text-xs">
                      {formatTimestamp(publisher.assignedAt, " ")}
                    </p>
                    <p
                      className={`w-[69px] text-center rounded-full px-1 text-white ${
                    ...
                    >
                      {capitalizeWords(publisher.status)}
                    </p>
                  </div>
                );
              })}
            ...
            <div className="border-b border-stone-[#e5e7eb] dark:border-[#8a8a8a] flex flex-col justify-between py-2">
              ...
              <p className="py-2">{version?.notes}</p>
            </div>
