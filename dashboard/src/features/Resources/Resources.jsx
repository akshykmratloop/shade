...
function Resources() {
  ...
  return (
    <div className="customscroller relative" ref={divRef}>
      <Navbar
        currentNav={currentResource}
        setCurrentResource={setCurrentResource}
      />
      <div
        className={`${resNotAvail ? "" : "grid"} ${
          isNarrow ? "grid-cols-1" : "grid-cols-2"
        } mt-4 lg:grid-cols-3 gap-10 w-full px-10`}
      >
        {resNotAvail ? (
          <p className="">Sorry, No Resource available for {currentResource}</p>
        ) : (
            ...
            <div key={index + Math.random()} className="w-full ">
              ...
              <div className="relative rounded-lg overflow-hidden border border-[1px] border-base-300 shadow-xl-custom">
                ...
                <div
                  className={` h-6 ${
                    page.assign
                      ? "bg-[#29469c] w-[120px]"
                      : "bg-red-600 w-[140px]"
                  } text-white flex items-center justify-center text-sm font-[300] clip-concave absolute top-3 left-0 z-10`}
                >
                ...
                <div className="relative aspect-[10/11] overflow-hidden">
                  <iframe
                    src={page.src}
                    className={`top-0 left-0 border-none transition-all duration-300 ease-in-out ${
                      isNarrow
                        ? "w-[1000px] scale-[0.5]"
                        : "w-[1200px] scale-[0.4]"
                ...
                {/* Bottom Text Options */}
                <div
                  className={`absolute bottom-3 left-0 w-full text-center text-white justify-center items-center flex ${
                  ...
                  {[
                      ...
                      onClick: () => {
                        page.subPage
                          ? page.subOfSubPage
                            ...
                            : settingRoute(currentResource, page.subPage)
                    ...
                    <span
                      ...
                      className={`flex ${isCollapsed ? "flex-col" : ""} ${
                        i < 2 ? "border-r-2 pr-5" : ""
                      } gap-2 items-center text-center cursor-pointer`}
                    >
                      {item.icon}
        ...
        {resources?.[currentResource]?.[0]?.subPage && (
          <div className="w-full flex flex-col gap-[5px] ">
            ...
            <div
              onClick={() => {
                navigate(
                  `./edit/${currentResource}/${
                    resources?.[currentResource].length + 1
                  }`
                );
              }}
